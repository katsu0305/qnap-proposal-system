...
**選定ロジックの流れ:**
1. `selection_rules` テーブルから条件に合致するルールを取得（優先度順）
2. 該当ルールの `recommended_model` から `products` テーブルの製品情報を取得
3. `industry_recommendations` テーブルから業種別追加推奨事項を取得
4. すべての情報を統合してレスポンス生成
...
[
  {
    ...
    "recommended_model": "TS-H1277AFX",
    "reason": "10GbE対応で超高速ファイル転送...",
    "is_active": true
  }
]
```

#### POST /api/selection-rules
選定ルール新規作成

#### PUT /api/selection-rules/:id
選定ルール更新

#### DELETE /api/selection-rules/:id
選定ルール削除
...
### 8. 選定ルール管理画面（admin/selection-rules.html）★新規追加★

**一覧表示:**
| ルール名 | 優先度 | 条件 | 推奨モデル | ステータス | アクション |
|---------|--------|------|-----------|----------|----------|
| 10GbE必須（大規模） | 100 | network:10gbe... | TS-H1277AFX | 有効 | 編集/削除 |
...
**重要性:**
このページがないと、製品マスタを変更しても選定ロジックに反映されません！
...
## 選定ロジックのマスタ管理に関する重要な注意事項 ★

### 問題点
WEB1では、選定ロジックがJavaScript内にハードコードされていました。
このままWEB2に移行すると、製品マスタをDBで管理しても選定結果に反映されません。

### 解決策
1. **選定ルールテーブル（selection_rules）の導入**
   - 条件（ユーザー数、予算、ネットワークなど）
   - 推奨モデル
   - 優先度
   - 推奨理由
...
2. **バックエンドで選定エンジン実装**
   - フロントエンドから条件を受け取る
   - selection_rulesテーブルから該当ルールを検索（優先度順）
   - productsテーブルから製品情報を取得
   - 統合した結果を返却
